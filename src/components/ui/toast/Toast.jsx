...
export function ToastProvider({ children }) {
  ...
  const showToast = useCallback(({ variant = "info", title, message, duration = 4000 }) => {
...
function Toaster({ toasts, onRemove }) {
  return (
    <div className="fixed top-4 right-4 z-[99999] flex flex-col gap-3 w-80 pointer-events-none">
      {toasts.map((toast) => {
        ...
        return (
          <div
            ...
            className={`relative overflow-hidden rounded-xl border p-4 shadow-lg pointer-events-auto ${styles.container}`}
          >
            <div className="flex items-start gap-3">
              <div className={`-mt-0.5 shrink-0 ${styles.icon}`}>{icons[toast.variant]}</div>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 break-words">{toast.message}</p>
              </div>
              <button
